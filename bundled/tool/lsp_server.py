...
LSP_SERVER = server.LanguageServer(
    name="Single Quote Formatter", version="0.1", max_workers=MAX_WORKERS
)
...
TOOL_DISPLAY = "Single Quote Formatter"
...
def _formatting_helper(document: workspace.Document) -> list[lsp.TextEdit] | None:
    result = quote_formatter.QuoteRewriter(document.source).rewrite()

    new_source = _match_line_endings(document, result)
    return [
        lsp.TextEdit(
            range=lsp.Range(
